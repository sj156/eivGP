---
title: "Toledo-inspired ADNI EIV-GP: frozen repeated validation experiment"
output:
  html_document:
    toc: true
    toc_depth: 3
...
The protocol exposes 20 explicit validation assignments: validations 1--9 are
exactly the former 3 repeats x 3 folds, represented through the same validation
interface, and validations 10--20 are prespecified random one-third
holdouts. Participants recur across validations, so these are not independent
studies. The first three legacy folds' role in earlier development should still be disclosed;
a fresh computational run does not erase earlier model selection. Data-driven
preprocessing and screening history remain necessary qualifications; freezing
data does not itself exclude leakage.

```{r setup, include=TRUE}
knitr::opts_chunk$set(echo = TRUE, message = TRUE, warning = TRUE)

## Environment variables override the notebook defaults without editing it:
## ADNI_VALIDATION_ID=10, EIVGP_N_CORES=4, or EIVGP_SMOKE_TEST=1.
...
VALIDATION_RAW <- if (PREPARE_ONLY && exists("ADNI_RUN_VALIDATION_ID", inherits = FALSE)) {
  as.character(get("ADNI_RUN_VALIDATION_ID", inherits = FALSE))
} else Sys.getenv("ADNI_VALIDATION_ID", "1")
if (!grepl("^[0-9]+$", VALIDATION_RAW)) stop("ADNI_VALIDATION_ID must be an integer.")
VALIDATION_ID <- as.integer(VALIDATION_RAW)
TOTAL_CORES <- env_int("EIVGP_CORES", env_int("EIVGP_N_CORES", 4L, 1L), 1L)
MAX_FOLD_WORKERS <- env_int("ADNI_MAX_FOLD_WORKERS", 3L, 1L)
SMOKE_TEST <- env_flag("EIVGP_SMOKE_TEST", FALSE)
stopifnot(VALIDATION_ID %in% 1:20)
...
VALIDATION_PLAN_FILE <- file.path(PROJECT_DIR, "validation_split_plan.csv")
...
OUTPUT_BASE <- Sys.getenv("ADNI_OUTPUT_DIR", file.path(PROJECT_DIR, "outputs"))
...
adni_write_csv(data.frame(schema = ADNI_CASE_SCHEMA, smoke = SMOKE_TEST,
  prediction_draws = PREDICTION_DRAWS, run_mode = "validation",
...
cat("Data:", DATA_DIR, "\n")
cat("Validation:", VALIDATION_ID, " | fit unit:", FOLDS_TO_RUN, "\n")
...
assignment <- read.csv(FOLD_FILE, stringsAsFactors = FALSE, check.names = FALSE)
manifest_file <- file.path(DATA_DIR, "validation-splits", "validation_file_manifest.csv")
if (!file.exists(manifest_file)) stop("Missing private validation_file_manifest.csv; rerun prepare_validation_splits.R")
private_manifest <- read.csv(manifest_file, stringsAsFactors = FALSE)
expected <- private_manifest[private_manifest$validation_id == VALIDATION_ID, , drop = FALSE]
if (nrow(expected) != 1L || unname(tools::md5sum(FOLD_FILE)) != expected$md5) {
  stop("Validation assignment MD5 is absent or does not match the private manifest.")
}
stopifnot(
  nrow(assignment) == 495L,
  !anyDuplicated(assignment$RID),
  all(assignment$validation_id == VALIDATION_ID),
  setequal(assignment$partition, c("train", "test")),
  sum(assignment$partition == "test") == VALIDATION_META$test_n,
  expected$source == VALIDATION_META$source,
  expected$generation_seed == if (VALIDATION_META$source == "legacy_balanced_3x3")
    VALIDATION_META$assignment_seed else VALIDATION_META$sampling_seed
)
folds <- data.frame(
  RID = assignment$RID,
  fold = ifelse(assignment$partition == "test", 1L, 0L)
)
...
stopifnot(
  ...
  setequal(unique(folds$fold), 0:1),
...
set_status <- function(run_dir, fold, stage, detail) {
  atomic_write_lines(c(
    ...
    paste0("validation=", VALIDATION_ID), paste0("fit_unit=", fold),
    paste0("stage=", stage), paste0("detail=", detail)
  ), file.path(run_dir, "CURRENT_STATUS.txt"))
  stamp(run_dir, paste("Validation", VALIDATION_ID, "|", stage, "|", detail))
}
...
draw_convergence_plots <- function(fit, diagnostic, fold_dir, stage) {
  draw_trace_panel(trace_series(fit),
                   ...
                   paste("Validation", VALIDATION_ID, stage, "key traces"))
  ...
  plot(seq_len(nrow(tab)), tab$rhat, pch = 16, col = cols[tab$group],
       ...
       main = paste("Validation", VALIDATION_ID, stage),
...
## Run validation `r VALIDATION_ID`
...
The scheduler divides the total core budget across validation jobs and chains.
Each validation has its own checkpoint and log. The validation lock remains
held by the coordinator while that job is active.
...
run_one_fold <- function(fold) {
  ...
  if (VALIDATION_META$source == "legacy_balanced_3x3") {
    legacy_repeat <- VALIDATION_META$legacy_repeat_id
    legacy_fold <- VALIDATION_META$legacy_fold_id
    fit_seed <- 202620000L + 100L * legacy_repeat + legacy_fold
    prediction_seed <- 202630000L + 100L * legacy_repeat + legacy_fold
    case_seed <- 202640000L + 100L * legacy_repeat + legacy_fold
  } else {
    fit_seed <- 202660000L + VALIDATION_ID
    prediction_seed <- 202670000L + VALIDATION_ID
    case_seed <- 202680000L + VALIDATION_ID
  }
  ...
  config <- data.frame(
    ...
    validation_id = VALIDATION_ID,
    fold = fold,
    assignment_source = VALIDATION_META$source,
    assignment_seed = if (VALIDATION_META$source == "legacy_balanced_3x3")
      VALIDATION_META$assignment_seed else VALIDATION_META$sampling_seed,
    legacy_repeat_id = VALIDATION_META$legacy_repeat_id,
    legacy_fold_id = VALIDATION_META$legacy_fold_id,
  ...
  atomic_write_lines(c(
    ...
    paste0("validation=", VALIDATION_ID),
  ...
  adni_extended_fold(fit, train, test, draw_ids, fold_dir, VALIDATION_ID, fold,
...
write.csv(combined_metrics, file.path(OUTPUT_ROOT, "validation_metrics.csv"), row.names = FALSE)
}
...
The primary table requires valid predictions from all four methods on the same
held-out participants. Incomplete results are labelled provisional and MCMC
warnings are retained.

```{r report, results='asis'}
if (!PREPARE_ONLY) {
  main_table <- adni_case_report(OUTPUT_ROOT, validation_id = VALIDATION_ID,
                                 smoke = SMOKE_TEST)
