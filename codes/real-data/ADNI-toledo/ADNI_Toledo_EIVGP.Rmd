...
Repeats 1, 2, and 3 can be selected from the frozen fold file. Repeat 1's
role in earlier development should still be disclosed when interpreting validation;
a fresh computational run does not erase earlier model selection. Data-driven preprocessing and screening history remain
...
REPEAT_ID <- if (PREPARE_ONLY) get("ADNI_RUN_REPEAT_ID", inherits = FALSE) else
  env_int("ADNI_REPEAT_ID", DEFAULT_REPEAT_ID, 1L)
...
stopifnot(REPEAT_ID %in% 1:3)

if (SMOKE_TEST) {
  N_CHAINS <- 4L; BURN <- 2L; INITIAL_ITER <- 8L
  EXTENSION_ITER <- 4L; CHECKPOINT_EVERY <- 3L
  FOLDS_TO_RUN <- if (env_flag("ADNI_SMOKE_ALL_FOLDS", FALSE)) 1:3 else 1L
  PREDICTION_DRAWS <- 8L
} else {
...
The total core budget is divided across folds and chains. A budget of 12 runs
three folds concurrently with four chain workers each. Each fold has its own
checkpoint and log. The repeat lock remains held by the coordinator; only the
coordinator writes combined tables and figures after all workers finish.
...
if (!PREPARE_ONLY) {
cat("Running folds; per-fold progress and errors are written to fold_N/worker.log.\n")
