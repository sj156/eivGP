...
if(any(args%in%c("help","--help"))){
 cat("Usage: Rscript recover_publication.R [plan|run|check] [--archive-root=DIR] [--data-root=DIR] [--output=DIR] [--studies=study1,study2] [--chain-workers=4]\n",
...
for(a in args[startsWith(args,"--")]){
 ...
 if(!key%in%c("archive-root","data-root","output","studies","chain-workers")||!is.null(options[[key]]))stop("Unknown or duplicated option: ",key)
 options[[key]]<-value
}
...
audit<-mixedgp_recover_publication(repo,archive,data,output,action,workers,studies)
