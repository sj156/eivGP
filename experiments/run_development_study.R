...
action <- if (length(args) > 1L) match.arg(args[2L], c("plan", "fit", "run")) else "plan"
...
rmarkdown::render(
  ...
  params = list(study = study, execute = action != "plan", fit_only = action == "fit",
                repository_dir = repo, core_budget = core_budget),
