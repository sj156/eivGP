...
```{r,eval=FALSE,include=FALSE,purl=FALSE}
#simu.results <- readRDS(params$results.file) From you own file
##

simu.results <- `EIV-ordinal-cos-10`
X <- simu.results$X 
y <- simu.results$y 
...
```{r fig.cap = 'Mixing plot for Tau', fig.height = 4.5*m, eval=FALSE,include=FALSE,purl=FALSE}
...
```{r fig.cap = 'Mixing plot for sigma.error2',eval=FALSE,include=FALSE,purl=FALSE}
...
```{r fig.cap = 'Mixing plot for u', echo = FALSE,  collapse = FALSE,eval=FALSE,include=FALSE,purl=FALSE}
idx <- 1
plot(U.hist[idx, ], 
     xlab = 'iteration', 
...
```{r ,echo = FALSE,eval=FALSE,include=FALSE,purl=FALSE}
...
```{r, fig.cap = 'Imputed u vs true u', echo = FALSE,eval=FALSE,include=FALSE,purl=FALSE}
ggfig <-
...
Gibbs.data <- list(
  ...
  y = y,
  X = X,
  X.star = W.grid[,1,drop = FALSE]
)

Gibbs.hist <- list(
  sigma.error2.hist = sigma.error2.hist[idx],
  U.hist = U.hist[,idx],
  U.star.hist = U.grid
)
...
pred.eiv <- impute.y(Gibbs.data,Gibbs.hist,GP.param,Gibbs.other)
#toc <- proc.time()[3]
#toc - tic  
...
predict.oracle <- predict.oracle(input.type = "ordinal", Gibbs.data, Gibbs.param, Gibbs.other)
...
Gibbs.hist <- list(
  beta.hist = beta.hist[,idx],
  sigma.u2.hist = sigma.u2.hist[idx],
  Tau.hist = Tau.hist[,idx]
)
...
## generate u.star
U.star.thin <- impute.u("ordinal",Gibbs.data,Gibbs.hist,Gibbs.other)
```
...
Gibbs.hist<- list(
  sigma.error2.hist = sigma.error2.hist[idx],
  U.hist = U.hist[,idx],
  U.star.hist = U.star.thin$U.star.hist
)
...
Gibbs.other <- list(seed = 888)

predict.result <- impute.y(Gibbs.data,Gibbs.hist,GP.param,Gibbs.other) #theta.eb, rho.eb,
#predict.result <- Predict.y(ncol(U.true), y, X, X.star, sigma.error2.hist[idx], U.hist[,idx], U.star.thin, theta.mle.obs, rho.mle.obs, seed = 888)
...
```{r fig.cap = 'Prediction visualization for y', fig.width= 3*8, fig.height = 6, echo = FALSE,eval=FALSE,include=FALSE,purl=FALSE}
...
```{r ,echo = FALSE,eval=FALSE,include=FALSE,purl=FALSE}
my_fill_scale <- scale_fill_manual(values = c("EIV" = "#1F78B4", "GP.unif" = "#33A02C", "GP.Gaussian" = "#FF7F00"))
#ggfig <-
