...
```{r}
#results.file <- "C:\Users\Lenovo\Desktop\eivGP\EIV-nominal-branin-gaussian-linear-50-100.rds"
#simu.results <- readRDS(results.file)

simu.results <- `EIV-nominal-branin-gaussian-linear-50-100`
```
...
U.oracle.pred <- predict.oracle(input.type = "nominal",Gibbs.data,Gibbs.param,Gibbs.other)
...
Gibbs.hist <- list(
  B.tilde.hist = B.tilde.hist[, idx.d],
  Sigma.u.hist = Sigma.u.hist[,idx.d],
  gamma.mat.hist = gamma.mat.hist[,idx.d1],
  sigma.error2.hist= sigma.error2.hist[idx],
  U.hist = U.hist[, idx.d],
  U.star.hist = U.star.thin
)
...
U.star.thin <- impute.u(input.type = "nominal",Gibbs.data,Gibbs.hist,Gibbs.other)
...
predict.result <- impute.y(Gibbs.data,Gibbs.hist,GP.param,Gibbs.other)
