...
if (!exists("STUDY1_STRICT_COMPETITORS")) {
  STUDY1_STRICT_COMPETITORS <- FALSE
}
...
rep_fits_file <- file.path(
  RES_DIR,
  "representative_eiv_fits_audited_tailstable_v3.rds"
)
...
############################################################
## Observed-input mean m(x,c): identifiable at every calibration size
############################################################

mean_x_grid <- seq(-2, 2, length.out = if (STUDY1_QUICK) 31L else 61L)
mean_grid <- expand.grid(x = mean_x_grid, c = seq_len(m))
mean_draw_cap <- if (STUDY1_QUICK) 50L else 120L
mean_latent_draws <- if (STUDY1_QUICK) 64L else 256L

mean_frames <- lapply(rep_fit_calibs, function(kk) {
  fit <- rep_fits[[as.character(kk)]]
  ids <- seq_len(nrow(fit$mcmc$samples_u))
  if (length(ids) > mean_draw_cap) {
    ids <- ids[unique(round(seq(1, length(ids), length.out = mean_draw_cap)))]
  }
  draws <- sample_eiv_m_given_xc_1d(
    x_star_raw = mean_grid$x,
    c_star = mean_grid$c,
    fit_obj = fit,
    draw_ids = ids,
  ...
  conditional_means <- attr(draws, "conditional_means")
  qs <- apply(draws, 2L, quantile, probs = c(0.025, 0.975), names = FALSE)
  transform(
    mean_grid,
    estimate = colMeans(conditional_means),
    lo = qs[1L, ],
    hi = qs[2L, ],
    truth = m0_1d(mean_grid$x, mean_grid$c, tau_true, rep_dat$scenario),
    calibration = paste0("|O| = ", kk),
    ordinal_level = paste0("c = ", mean_grid$c)
  )
})
...
mean_df$ordinal_level <- factor(
  mean_df$ordinal_level,
  levels = paste0("c = ", seq_len(m))
)

p_mean_function <- ggplot(mean_df, aes(x = x)) +
  ...
  facet_grid(ordinal_level ~ calibration) +
  labs(
    x = "x",
    y = expression(m(x, c)),
    title = "Recovery of the observed-input mean",
    subtitle = "Orange: truth; blue: posterior mean and pointwise 95% band"
  )

ggsave(
  file.path(FIG_DIR, "fig_study1_mean_function_recovery.pdf"),
  p_mean_function,
  width = 10.5,
  height = 11
)
...
competitor_selected <- run_study1_published_competitors(
  X_train = matrix(train$x, ncol = 1L),
  y_train = train$y,
  C_train = matrix(train$c, ncol = 1L),
  X_test = matrix(selected_grid$x_star, ncol = 1L),
  C_test = matrix(selected_grid$c_star, ncol = 1L),
  m_vec = m,
  ...
  controls = list(
    ...
    EzGP = list(maxeval = if (STUDY1_QUICK) 30L else 100L)
  )
)
write.csv(
  competitor_selected$status,
  file.path(TAB_DIR, "study1_representative_competitor_status.csv"),
  row.names = FALSE
)
...
df_density <- dplyr::bind_rows(c(
  list(
    make_long_draws(draws_oracle_selected, "Oracle", selected_grid),
    make_long_draws(draws_eiv_selected, "EIV-GP", selected_grid)
  ),
  lapply(names(competitor_selected$draws), function(method) {
    make_long_draws(competitor_selected$draws[[method]], method, selected_grid)
  })
))

df_density$method <- factor(
  df_density$method,
  levels = c("Oracle", "EIV-GP", "UC-GP", "LVGP", "EzGP")
)
