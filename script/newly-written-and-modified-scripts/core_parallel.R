...
mixedgp_adaptive_mcmc_control <- function(initial_draws = 5000L,
                                           target_ess = 200,
                                           max_draws = 15000L,
                                           extension_draws = 1000L) {
  initial_draws <- mixedgp_as_integer_strict(
    initial_draws, "initial_draws", min_value = 1L, length_expected = 1L
  )
  max_draws <- mixedgp_as_integer_strict(
    max_draws, "max_draws", min_value = initial_draws,
    length_expected = 1L
  )
  extension_draws <- mixedgp_as_integer_strict(
    extension_draws, "extension_draws", min_value = 1L,
    length_expected = 1L
  )
  if (!is.numeric(target_ess) || length(target_ess) != 1L ||
      !is.finite(target_ess) || target_ess <= 0) {
    stop("target_ess must be one positive finite number.")
  }
  list(
    initial_draws = initial_draws,
    target_ess = as.numeric(target_ess),
    max_draws = max_draws,
    extension_draws = extension_draws
  )
}

mixedgp_adaptive_next_draws <- function(current_draws,
                                         observed_ess,
                                         control,
                                         n_chains = 1L) {
  current_draws <- mixedgp_as_integer_strict(
    current_draws, "current_draws", min_value = 1L, length_expected = 1L
  )
  n_chains <- mixedgp_as_integer_strict(
    n_chains, "n_chains", min_value = 1L, length_expected = 1L
  )
  if (!is.list(control)) stop("control must be an adaptive-MCMC control list.")
  control <- do.call(mixedgp_adaptive_mcmc_control, control)
  target_per_chain <- control$target_ess / n_chains
  if (is.finite(observed_ess) && observed_ess >= target_per_chain) {
    return(list(
      next_draws = current_draws, target_per_chain = target_per_chain,
      reason = "ess_target_met"
    ))
  }
  projected <- if (is.finite(observed_ess) && observed_ess > 0) {
    ceiling(current_draws * target_per_chain / observed_ess)
  } else {
    current_draws + control$extension_draws
  }
  projected <- max(projected, current_draws + control$extension_draws)
  projected <- ceiling(projected / control$extension_draws) *
    control$extension_draws
  next_draws <- min(control$max_draws, projected)
  list(
    next_draws = as.integer(next_draws),
    target_per_chain = target_per_chain,
    reason = if (next_draws >= control$max_draws) {
      "extension_to_cap"
    } else {
      "ess_projected_extension"
    }
  )
}
